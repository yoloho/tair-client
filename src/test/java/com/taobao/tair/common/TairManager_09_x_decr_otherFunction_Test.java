...
public class TairManager_09_x_decr_otherFunction_Test extends BaseTest {
	private int namespace = 1;
	@Test
	public void test_901_decr_interface_otherFunction_Test_qa() {   
        //step 1 add data to test
		String key1 = "901_key_1";
        int value1 = 1;
        tairManager.delete(namespace, key1);
        Result<Integer> count = tairManager.decr(namespace, key1, value1, 0, 10);
        ...
        String key2 = "901_key_2";
        int value2 = 3;
        tairManager.delete(namespace, key2);
        count = tairManager.decr(namespace, key2, value2, 0, 10);
        Assert.assertTrue(count.isSuccess());
        Assert.assertEquals(ResultCode.SUCCESS,count.getRc());
        ...
        Assert.assertTrue(-1==(Integer)((DataEntry) results.getValue().get(1)).getValue()||-1==(Integer)((DataEntry) results.getValue().get(0)).getValue());
        Assert.assertTrue(1==((DataEntry) results.getValue().get(1)).getVersion()||1==((DataEntry) results.getValue().get(0)).getVersion());
        Assert.assertTrue(key1.equals(((DataEntry) results.getValue().get(1)).getKey().toString())||key1.equals(((DataEntry) results.getValue().get(0)).getKey().toString()));
        Assert.assertTrue(-3==(Integer)((DataEntry) results.getValue().get(1)).getValue()||-3==(Integer)((DataEntry) results.getValue().get(0)).getValue());
        Assert.assertTrue(1==((DataEntry) results.getValue().get(1)).getVersion()||1==((DataEntry) results.getValue().get(0)).getVersion());
        Assert.assertTrue(key2.equals(((DataEntry) results.getValue().get(1)).getKey().toString())||key2.equals(((DataEntry) results.getValue().get(0)).getKey().toString()));
	  ...
	  //step 9 getItemsCount
        ...
        //step 13 decr new data
        count = tairManager.decr(namespace, key1, value1, 0, 10);
        Assert.assertTrue(count.isSuccess());
        Assert.assertEquals(ResultCode.SUCCESS,count.getRc());
        Assert.assertEquals(-1,count.getValue().intValue());
